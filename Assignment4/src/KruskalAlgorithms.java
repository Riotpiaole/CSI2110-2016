...
public class KruskalAlgorithms<V,E extends Comparable<E>> {
	...
	public LinkedList <Edge<E>>  kruskalClusters (Graph<V,E> g, int k, Partition<Vertex<V>> clusters) {
		...
		while(!pq.isEmpty() && clusters.numClusters() != k){
			Vertex<V>[]temp= (Vertex<V>[]) g.endVertices(pq.poll());
			if(clusters.find(hash.get(temp[0])) != clusters.find(hash.get(temp[1])) ){
				forest.addLast(g.getEdge(temp[0],temp[1]));
				clusters.union(hash.get(temp[1]),hash.get(temp[0]));
			}
		}
